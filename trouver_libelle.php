...
if(isset($_GET["single"]))
{
	...
	$sql="SELECT libelle, var_colonne, color FROM t_graphique_structure WHERE variable='".$variable."' AND libelle NOT LIKE 'effectif' AND libelle NOT LIKE 'NPS' AND analyse='1' ORDER BY ordre";
	$requete = $bdd->query($sql);

	while($liste = $requete->fetch())
	{
		
		$sqlvaleur = "SELECT `".$liste["var_colonne"]."` FROM t_graphique WHERE 1=1 ".$axe.$frequence.$vague;
...
elseif(isset($_GET["variable"]))
{
	...
	$sql="SELECT var_colonne FROM t_graphique_structure WHERE variable='".$variable."' AND evolution='1' ORDER BY ordre";
$requete = $bdd->query($sql);

	$liste = $requete->fetch();

		$sqlvaleur = "SELECT vague FROM t_graphique WHERE `".$liste["var_colonne"]."` IS NOT NULL".$typevague.$axe.$frequence." GROUP BY vague";
		$requetevaleur = $bdd->query($sqlvaleur);
		while($listevaleur = $requetevaleur->fetch())
		{
...
elseif(isset($_GET["valeur"]))
{
	...
	// CALCUL DES VAGUES
	/*
	$sql="SELECT var_colonne FROM t_graphique_structure WHERE variable='".$variable."' AND libelle NOT LIKE 'effectif' AND analyse='1' AND ordre='1'";
	$requete = $bdd->query($sql);
	$liste = $requete->fetch();

		$sqlvaleur = "SELECT vague FROM t_graphique WHERE ".$liste["var_colonne"]." IS NOT NULL".$axe." GROUP BY vague";
		$requetevaleur = $bdd->query($sqlvaleur);
		while($listevaleur = $requetevaleur->fetch())
		{
	...
	*/
	...
	$sql="SELECT libelle, var_colonne, color FROM t_graphique_structure WHERE variable='".$variable."' AND evolution='1' ORDER BY ordre";
	$requete = $bdd->query($sql);
	while($liste = $requete->fetch())
	{
		$sqlvaleur = "SELECT `".$liste["var_colonne"]."`, vague FROM t_graphique WHERE 1=1 ".$typevague.$axe.$frequence." GROUP BY vague";
		$requetevaleur = $bdd->query($sqlvaleur);
		$table1 = array();
		while($listevaleur = $requetevaleur->fetch())
		{
...
elseif(isset($_GET["effectif"]))
{
	...
	$sql="SELECT libelle, var_colonne FROM t_graphique_structure WHERE variable='".$effectif."' AND libelle LIKE 'effectif' AND analyse='1' LIMIT 1";
	$requete = $bdd->query($sql);
	$liste = $requete->fetch();
	...
	if(!empty($liste))
	{
		...
		$sqlvaleur = "SELECT `".$liste["var_colonne"]."` FROM t_graphique WHERE 1=1 ".$vague.$axe.$frequence." LIMIT 1";
		//echo $sqlvaleur;
		$requetevaleur = $bdd->query($sqlvaleur);
		$listevaleur = $requetevaleur->fetch();
		if($listevaleur[$liste["var_colonne"]]<30)
		{	$data = "⚠ Base répondants Faible: ".$listevaleur[$liste["var_colonne"]];}
		else
	...
	$sqlnps="SELECT libelle, var_colonne FROM t_graphique_structure WHERE variable='".$effectif."' AND libelle LIKE 'NPS' AND analyse='1'";
	$requetenps = $bdd->query($sqlnps);
	//$listenps = $requetenps->fetch();
	while($listenps = $requetenps->fetch())
	{
		if(!empty($listenps))
		{
			// TODO récupérer la valeur correspondant aux var_colonne
			$sqlvaleurnps = "SELECT `".$listenps["var_colonne"]."` FROM t_graphique WHERE 1=1 ".$vague.$axe.$frequence." LIMIT 1";
			//echo $sqlvaleur;
	...
	$sqlindic="SELECT libelle, var_colonne FROM t_graphique_structure WHERE variable='".$effectif."' AND indicateur='1' ORDER BY ordre";
	...
	while($listeindic = $requeteindic->fetch())
	{
		...
		{
			// TODO récupérer la valeur correspondant aux var_colonne
			$sqlvaleurindic = "SELECT `".$listeindic["var_colonne"]."` FROM t_graphique WHERE 1=1 ".$vague.$axe.$frequence." LIMIT 1";
